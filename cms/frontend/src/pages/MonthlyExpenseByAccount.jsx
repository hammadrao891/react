...
const fetchExpenses=async()=>{
    ...
    try
    {
        const response = await axios({
            method:"get",
        baseURL:"http://localhost:8000/api/",
        url:`/expense/expenses_by_account_type/${month < 10  &&  !(/^\d{2}$/.test(month)) ? "0"+month :month }/${year}/${accountType}`,
        })
...
<label style={{color:"black",fontSize:"small"}}>Select Month and Year :</label>
           ...
           <input
            ...
            <select name="Acct_type_id" onChange={e=>setAccountType(e.target.value)}>
...
export default MonthlyExpenseByAccount;
