...
const CreateTimeTable = () => {
  ...
  const handleChange = async (e, index, day, timeSlot, dayToCheck) => {
    ...
    if (classMatch.match)
      ...
      const indexToUpdateExists = day?.some(subarray => subarray[1] === timeSlot);

      if (indexToUpdateExists) {
        // Update the existing subarray
        day.forEach(subarray => {
          if (subarray[1] === timeSlot) {
            subarray[0] = e.target.value;
            subarray[1] = timeSlot
          }
        });
      } else {
        // Push a new subarray
        day.push([e.target.value, timeSlot]);
      }
  ...
  useEffect(() => {
    ...
    const handleButtonClick = async (day) => {
      const updatedDay = [...day]; // Make a copy of the day array
      const currentLength = updatedDay.length;

      if (currentLength < 7) {
        // Fill the remaining slots with "-" and corresponding time slots
        for (let i = currentLength; i < 7; i++) {
          updatedDay.push(["-", timeArr[i]]);
        }
      }
      let newSchedule;
      switch (day) {
        case monday:
          ...
          setMonday(updatedDay);
          break;
        case tuesday:
          setTuesday(updatedDay);
          break;
        case wednesday:

          setWednesday(updatedDay);
          break;
        case thursday:

          setThursday(updatedDay);
          break;
        case friday:

          setFriday(updatedDay);
          break;
        default:
          break;
      }
